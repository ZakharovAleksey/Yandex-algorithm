...
{
	MySplayNode<T>* left_;
	MySplayNode<T>* right_;
	MySplayNode<T>* parent_;
	T key_;


	MySplayNode() : left_(nullptr), right_(nullptr), parent_(nullptr), key_(T()) {}
	MySplayNode(T key) : left_(nullptr), right_(nullptr), parent_(nullptr), key_(key) {}
};
...
{
public:
	MySplayTree() : root_(nullptr) {}
	~MySplayTree() { delete root_; }
...
private:
	...
	void InorderTraverse(MySplayNode<T>* curNode);
...
template<typename T>
inline void MySplayTree<T>::Insert(T key)
{
	if (!root_)
	{
		root_ = new MySplayNode<T>(key);
	}
	else
	{
		MySplayNode<T>* curNode = root_;
		MySplayNode<T>* parent = curNode->parent_;
		...
		if (key < parent->key_)
		{
			parent->left_ = new MySplayNode<T>(key);
			parent->left_->parent_ = parent;
		}
		else
		{
			parent->right_ = new MySplayNode<T>(key);
...
template<typename T>
inline void MySplayTree<T>::InorderTraverse()
{
	InorderTraverse(root_);
}

template<typename T>
inline void MySplayTree<T>::InorderTraverse(MySplayNode<T>* curNode)
{
	if (curNode->left_)
		InorderTraverse(curNode->left_);
